...
export default class Pict {
  ...
  generateCliArgs() {
    ...
    if (this.pictOptions.order_of_combinations) {
      cli = `${cli} /o:${this.pictOptions.order_of_combinations}`;
    }

    if (this.pictOptions.randomize_generation) {
      cli = `${cli} /r:${this.pictOptions.randomize_generation}`;
    }

    if (this.pictOptions.case_sensitive_model_evaluation) {
      cli = `${cli} /c`;
    }

    if (this.pictOptions.show_model_statistics) {
      cli = `${cli} /s`;
    }
